...
const SectionRenderer: React.FC<SectionRendererProps> = ({ item, section }) => {
    switch (item.type) {
        case 'paragraph':
            return <p className="mb-7 text-lg text-brand-text-light leading-[1.8]">{formatText(item.content as string)}</p>;
        case 'heading':
            return <h3 className="text-xl md:text-2xl font-outfit font-black text-brand-text mb-5 mt-10 tracking-tight">{formatText(item.content as string)}</h3>;
        case 'quote':
            return (
                <blockquote className="relative border-l-4 border-brand-primary pl-6 my-8 py-2">
        ...
        case 'image':
            return <img src={item.content as string} alt={item.alt || ''} className="my-8 rounded-2xl shadow-elevated" />;
        case 'interactive':
            if (item.component) {
                        ...
                        <div className="my-8">
                            <div className="flex items-center gap-3 mb-4 pl-2">
                                <div className="w-10 h-10 flex items-center justify-center bg-brand-bg rounded-full shadow-neumorphic-out">
                                    <LabsIcon className="w-6 h-6 text-brand-cyan" isAnimating={true} />
                                </div>
                                <h4 className="font-outfit font-bold text-lg text-brand-text">Interactive Lab</h4>
                            </div>
                            <Suspense fallback={
                                <div className="glass-card p-12 flex flex-col items-center justify-center gap-3">
                                    ...
                                    <p className="text-sm text-brand-text-light font-medium">Loading Interactive...</p>
                                </div>
                            }>
            ...
            return <div className="text-red-500 my-8 p-6 bg-red-50 border border-red-200 rounded-2xl">Error: Interactive component "{item.component}" not found.</div>;
        default:
            return null;
    }
};
